...
export default function Blog({ type = 'blog' }) {
  const label = type === 'article' ? 'Articles' : 'News & Blog';
  const base = type === 'article' ? '/articles' : '/blog';
  const [posts, setPosts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');

  useEffect(() => { document.title = `${label} | JT NextGen Tech Hub`; }, [label]);

  useEffect(() => {
    let active = true;
    setLoading(true); setError(''); setPosts([]);
    blogAPI.getAll({type}).then((r) => { if (active) setPosts(r.data || []); }).catch(() => { if (active) setError('Unable to load posts. Please reload this page to try again.'); }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [type]);
      ...
      <section className="py-12 md:py-20 px-4 md:px-6 bg-gray-50 dark:bg-gray-800/50 min-h-screen">
        <div className="max-w-6xl mx-auto">
          <motion.div {...fadeUp} className="text-center mb-12">
            <h1 className="text-4xl font-bold text-brand-800 mb-4">{label}</h1>
          ...
          {error ? <p role="alert" className="text-center py-12">{error}</p> : loading ? (
            ...
            <p className="text-center text-gray-400 py-20">No published {type === 'article' ? 'articles' : 'blog posts'} yet. Check back soon!</p>
          ) : (
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {posts.map((p, i) => (
                <motion.div key={p.id} {...fadeUp} transition={{ delay: i * 0.05 }}>
                  <Link to={`${base}/${p.slug}`} className="accent-card block bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md hover:border-brand-200 transition-all group">
